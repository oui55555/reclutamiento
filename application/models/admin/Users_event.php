...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Users_event extends CI_Model {
	...
	function add_users(){
			...
			$insert = $this->db->insert('users_event', $new_member_data);
			return $insert;
	...
	function update_users($seg){
                ...
                $query= $this->db->from('users_event')
                        ->where('user_event_id', $this->input->post('user_event_id'))
                        ->update('users_event', $user_data);

                if($query)
                {
			        redirect($url.'/admin_home/valid_user/users_event/0/'.$seg);
                    return true;
                }
                else
                {
	...
	function delete_users($id, $seg){
        $this->db->delete('users_event', array('user_event_id'=> $id));
